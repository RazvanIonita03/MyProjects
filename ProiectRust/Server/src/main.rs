            ...
            Ok(bytes_read) if bytes_read > 0 => {
                ...
                if message.starts_with("The machine is already connected") {
                                ...
                                if let Some(user_name) = user.get("username") {
                                    if user_name == username {
                                        if let Some(token) = user.get("token") {
                                            ...
                                            let user_token_trimmed =
                                                user_token.trim().trim_matches('"');
                    ...
                    if register_success {
                        ...
                        let response =
                            "Registration has failed. The username already exists in the database";
                        if let Err(e) = stream.write_all(response.as_bytes()) {
                            eprintln!("Failed to write to stream: {}", e);
                        }
                    }
                                ...
                                if let Some(user_name) = user.get("username") {
                                    ...
                                    if user_name == currentuser_v {
                                        if let Some(metadata) = user.get("metadata") {
                                            ...
                                            if let Some((output, _)) = metadata_map.get(token) {
                                                response_body = format!("Command output: {}", output);
                                            } else {
                                                eprintln!("Token not found in metadata for user: {}", currentuser_v);
                                            }
                                        } else {
                                            eprintln!("Metadata not found for user: {}", currentuser_v);
                                        }
                                    } else {
                                        eprintln!("User does not match current user: {}", user_name);
                                    }
                                } else {
                                                        ...
                                                        match run_piped_command(command){
                                                            Ok(output) => {
                                                                if output == "Command executed with no output." {
                                                                    ...
                                                                    match metadata_map.insert(random_token.clone(),(output.to_string(), time_called.to_string()),) {
                                                                        Some(_) => {
    ...
    match Command::new("cmd").arg("/C").arg(command).output() {
        Ok(output) => {
            if !output.stdout.is_empty() {
                ...
                Ok(format!("Command error: {}", String::from_utf8_lossy(&output.stderr)
            ))
            } else {
